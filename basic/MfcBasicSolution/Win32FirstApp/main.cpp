...
int WINAPI WinMain(
	HINSTANCE hInstance,		// 현재 프로그램 객체인스턴스(핸들)
	...
	while (GetMessage(&msg, NULL, 0, 0)) {	// 메시지가 대기
		TranslateMessage(&msg);		// 메시지 변환
		DispatchMessage(&msg);		// 메시지 처리 WndProc() 함수로 전달
	}
...
LRESULT CALLBACK WndProc(
	...
	case WM_DESTROY:	// 윈도우 종료 메시지가 넘어오면 
		PostQuitMessage(0);		// GetMessage() 0 으로 처리
		return 0;
	}
